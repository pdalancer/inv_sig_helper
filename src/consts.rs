...
pub static REGEX_SIGNATURE_FUNCTION_PATTERNS: &[&str] = &[
    ...
    r#"([a-zA-Z0-9_$]{1,})=function\(([a-zA-Z0-9_$]{1})\)\{[^{}]*GLOBAL_VAR_NAME\[\d+\][^{}]*GLOBAL_VAR_NAME\[\d+\][^{}]*return[^{}]*GLOBAL_VAR_NAME\[\d+\][^{}]*\}"#, // requires multiple GLOBAL_VAR_NAME references without control flow
    r#"([a-zA-Z0-9_$]{1,})=function\(([a-zA-Z0-9_$]{1})\)\{[^}]*GLOBAL_VAR_NAME\[[^\]]+\][^}]*return [^}]*GLOBAL_VAR_NAME\[[^\]]+\][^}]*\}"#, // basic pattern with GLOBAL_VAR_NAME usage
    r#"([a-zA-Z0-9_$]{1,})=function\(([a-zA-Z0-9_$]{1})\)\{[^}]*return [^}]*GLOBAL_VAR_NAME\[[^\]]+\][^}]*\}"#, // simple return pattern with GLOBAL_VAR_NAME
];
